...
export function AnnouncementBar() {
  return (
    <div className="sticky top-0 z-50 overflow-hidden bg-lime-400 pt-[env(safe-area-inset-top)] text-background">
      <div className="flex h-7 items-center">
        <div className="flex w-max shrink-0 animate-ticker items-center gap-8 pl-8 motion-reduce:animate-none">
          {[...ITEMS, ...ITEMS].map((item, i) => (
            ...
            >
              {item}
              <span className="size-1 rounded-full bg-background" aria-hidden />
            </span>
          ))}
        </div>
      </div>
